...
public abstract class SuportadoAgrupavelBonus extends SuportadoAgrupavel {
    ...
    private void aplicarDestruicaoCombinada() {
        ...
        for (int i = linha-1 ; i <= linha+1; i++) {
            adicionaCasoNaoExista(bases, baseSuportadora.getAreaJogavel().getBasesSuporteDaLinha(i));
        }

        int coluna = baseSuportadora.getPosicao().getColuna();
        for (int i = coluna-1 ; i <= coluna+1; i++) {
            adicionaCasoNaoExista(bases, baseSuportadora.getAreaJogavel().getBasesSuporteDaColuna(i));
        }
    ...
    private void adicionaCasoNaoExista(List<BaseSuportadora> bases, List<BaseSuportadora> basesSuporteNovas) {
        for (BaseSuportadora baseSuportadora: basesSuporteNovas){
